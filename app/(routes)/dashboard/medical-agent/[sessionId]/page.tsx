...
const MedicalVoiceAgent = () => {
  ...
  const startCall = () => {
    ...
    const VapiAgentConfig={
      ...
      firstMessage:"Hi there! I'm your AI Medical Assistant. I am here to help you with any health questions or concerns you might have today. How are you feeling?",
      transcriber:{
    ...
    vapi.on("message", (message) => {
      if (message.type === "transcript") {
        ...
        if(transcriptType=='partial'){
          ...
          setmessage((prev: messages[] = []) => [...prev, { role: role, text: transcript }]);
          setLiveTranscripts("")
          setCurrentRole(null)
        }
      }
   ...
   const endCall = () => {
    ...
    console.log(vapiInstance);
      vapiInstance.stop();
      vapiInstance.off('call-start', () => {console.log("Call started"),setCallStarted(true)});
      vapiInstance.off('call-end', () => {console.log("Call ended"),setCallStarted(false)});
    ...
    <div className="p-5 border rounded-3xl bg-secondary shadow-md ">
      <div className="flex items-center justify-between p-4">
        <h2 className="p-1 px-2 border rounded-md flex gap-2 items-center">
          <Circle className={`h-4 w-4 rounded-full text-white ${callStarted ? 'bg-green-500' : 'bg-red-500'}`} />
        ...
        <div className="flex flex-col items-center mt-10">
          ...
          <div className="mt-32 overflow-y-auto flex flex-col items-center px-10 md:px-28 lg:px-52 xl:px-72">
            {message?.slice(-4).map((msg,index)=>(
              <h2 className="text-gray-400 p-1 text-center" key={index}>{msg.role} : {msg.text}</h2>
            ))}
            {liveTranscripts && liveTranscripts?.length>0 && <h2 className="text-lg">{currentRole} : {liveTranscripts}</h2>}
          </div>
          {!callStarted ? <Button className="mt-20" onClick={startCall} disabled={callStarted}>
            <PhoneCall />
            Start Call
          </Button>
          : (
